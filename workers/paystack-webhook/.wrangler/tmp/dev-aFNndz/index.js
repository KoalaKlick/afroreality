...
// .wrangler/tmp/bundle-86yBQy/strip-cf-connecting-ip-header.js
...
// src/index.ts
async function verifyPaystackSignature(signature, bodyText, secret) {
  if (!signature || !secret)
    return false;
  try {
    ...
    try {
      ...
      if (eventType === "charge.success" && data.status === "success") {
        ...
        await sql`
					...
					WHERE reference = ${reference}
				`;
        if (env2.DELIVERY_QUEUE) {
          await env2.DELIVERY_QUEUE.send({
      ...
      console.error("Paystack Webhook Worker Error:", error3);
...
// .wrangler/tmp/bundle-86yBQy/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-86yBQy/middleware-loader.entry.ts
