...
// .wrangler/tmp/bundle-wQHtsi/strip-cf-connecting-ip-header.js
...
// src/index.ts
var MAX_LISTED_EVENTS = 7;
var ENTER_CODE_OPTION = "0";
function normalizePhone(phone) {
  let localPhone = phone.replace(/^\+?233/, "0");
  if (localPhone.startsWith("00"))
...
__name(reduceTokens, "reduceTokens");
function getPaginatedSelection(tokens) {
  let page = 1;
  const PAGE_SIZE = 7;
...
__name(getPaginatedSelection, "getPaginatedSelection");
function buildPaginatedMenu(title2, items, page, renderItem) {
  const PAGE_SIZE = 7;
...
__name(buildPaginatedMenu, "buildPaginatedMenu");
async function fetchEventByCode(sql, code) {
...
__name(fetchEventByCode, "fetchEventByCode");
async function fetchEventDetails(sql, eventId) {
...
__name(fetchEventDetails, "fetchEventDetails");
async function submitOtp(sql, reference, otp, paystackSecret) {
  try {
...
__name(submitOtp, "submitOtp");
async function processPayment(sql, event, optionId, quantity, price, phoneNumber, paystackSecret, otpStr) {
...
async function handleVotingFlow(sql, event, details, tokens, phoneNumber, paystackSecret) {
...
`
  ...
  if (!selectedCategory)
    return textResponse("END Invalid category.");
  tokens = catSelection.remainingTokens;
  const nominees = (details.options || []).filter(
    (n) => n.category_id === selectedCategory.id
  );
...
`
  ...
  if (!selectedNominee)
    return textResponse("END Invalid nominee.");
...
__name(handleVotingFlow, "handleVotingFlow");
async function handleTicketFlow(sql, event, details, tokens, phoneNumber, paystackSecret) {
...
`
  ...
  if (!selectedTicket)
    return textResponse("END Invalid ticket.");
...
__name(handleTicketFlow, "handleTicketFlow");
async function handleUssdCore(phoneNumber, text, env2) {
  const dbUrl = env2.DATABASE_URL;
  const paystackSecret = env2.PAYSTACK_SECRET_KEY || "";
  if (!dbUrl) {
...
0. Cancel`
  ...
  if (depth === 0) {
    const maxEvents = Number.parseInt(
      env2.MAX_LISTED_EVENTS || `${MAX_LISTED_EVENTS}`,
      10
    );
    const events = await sql`
...
__name(handleUssdCore, "handleUssdCore");
async function toArkeselResponse(atResponse, sessionID, userID, msisdn) {
...
__name(toArkeselResponse, "toArkeselResponse");
function normalizeArkeselInput(userData, newSession) {
...
__name(normalizeArkeselInput, "normalizeArkeselInput");
var src_default = {
  async fetch(request, env2) {
    ...
    try {
      ...
      const isArkeselRoute = url.pathname.includes("/arkesel") || url.searchParams.get("provider") === "arkesel";
      let bodyData = {};
      let rawText = "";
      let phoneNumber = "";
      let sessionId = "";
      ...
      if (contentType.includes("application/json")) {
        ...
        rawText = bodyData.text !== void 0 ? bodyData.text : bodyData.userData || bodyData.message || "";
        ...
        phoneNumber = formData.get("phoneNumber") || formData.get("msisdn") || "";
        rawText = formData.get("text") || formData.get("userData") || formData.get("message") || "";
        sessionId = formData.get("sessionId") || formData.get("sessionID") || "";
        userId = formData.get("userId") || formData.get("userID") || "";
        const typeStr = (formData.get("type") || "").toLowerCase();
        isNewSession = formData.get("newSession") === "true" || typeStr === "initiation";
      } else {
        phoneNumber = url.searchParams.get("phoneNumber") || url.searchParams.get("msisdn") || "";
        rawText = url.searchParams.get("text") || url.searchParams.get("userData") || "";
      ...
      if (isArkeselRoute || bodyData.sessionID || bodyData.msisdn) {
        ...
        if (env2.DATABASE_URL && sessionId) {
          const sql = Xs(env2.DATABASE_URL);
          if (!isNewSession) {
      ...
      return await handleUssdCore(phoneNumber, rawText, env2);
    } catch (error3) {
      console.error("USSD Worker Error:", error3);
...
// .wrangler/tmp/bundle-wQHtsi/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-wQHtsi/middleware-loader.entry.ts
