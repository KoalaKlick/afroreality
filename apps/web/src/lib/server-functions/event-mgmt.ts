...
export async function createNewEvent({ data }: { data: any }) {
  ...
  if (!organizationId) {
    throw new Error('Please create or select an organization before creating events.');
  }
  ...
  const event = await prisma.event.create({
    data: {
      ...
      socialLinks: validSocialLinks.length > 0 ? { create: validSocialLinks } : undefined,
      galleryLinks: validGalleryLinks.length > 0 ? { create: validGalleryLinks } : undefined,
    },
