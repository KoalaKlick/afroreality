...
export async function getOrganizationById({ data }: { data: { id: string } }): Promise<any> {
  const session = await requireSession();
...
export async function createOrganizationAccount({ data }: { data: any }): Promise<any> {
  const session = await requireSession();
...
export async function updateOrganizationSettings({ data }: { data: any }): Promise<any> {
...
export async function checkOrgSlug({ data }: { data: { slug: string } }): Promise<{ available: boolean }> {
