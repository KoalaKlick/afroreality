...
import { generateSlug, isReservedSlug, organizationNameSchema, organizationSlugSchema } from "@/lib/validations/organization";
...
export function Step4OrgInfo({
  ...
  useEffect(() => {
    ...
    checkOrgSlug({ data: { slug: debouncedSlug } })
      .then((result) => {
        if (result.available || debouncedSlug === defaultValues?.slug?.toLowerCase()) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!nameParse.success) {
      toast.error(nameParse.error.issues[0]?.message ?? "Invalid organization name");
      return;
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-4 md:space-y-5">
        ...
        <div className="space-y-1.5">
          <div className="flex items-center justify-between">
            <label className="text-sm font-medium leading-none">URL Handle</label>
          ...
          <p className="text-xs text-muted-foreground">
            Portal URL: <span className="font-semibold text-foreground">afroreality.com/{slug || "handle"}</span>
          </p>
        </div>
